    ...
    def check_val_accuracy(self):
        ...
        for x, y, ans, idx in self.val_loader:
            ...
            for i in range(len(y)):
                if list(y[i][y[i].nonzero().squeeze().detach()][1:]) == list(pred[i][pred[i].nonzero().squeeze().detach()]):
                    aux +=1
                elif list(y[i][y[i].nonzero().squeeze().detach()][1:]) == list(pred[i][pred[i].nonzero().squeeze().detach()])[:len(y[i]) -1]:
                    aux +=1
            reward += (aux /pred.shape[0])
